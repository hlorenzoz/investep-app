  ...
  group('CompoundInterestCalculator Tests', () {
    ...
    void verifySeriesIntegrity(List<CompoundInterestPeriodResult> series) {
      for (int i = 0; i < series.length; i++) {
        ...
        final expectedEnd = double.parse(
          (p.startBalance + p.yieldAmount).toStringAsFixed(2),
        );
        expect(
          p.endBalance,
          equals(expectedEnd),
          reason:
              'Error aritmético en el período ${p.label} (${p.periodIndex})',
        );
        ...
        if (i > 0) {
          ...
          expect(
            p.startBalance,
            equals(prev.endBalance),
            reason:
                'Discrepancia de continuidad entre ${prev.label} y ${p.label}',
          );
        }
      }
    }
    ...
    test(
      'Cálculo Diario: verificación de fórmula, crecimiento e integridad de serie',
      () {
    ...
    test(
      'Cálculo Semanal: verificación de fórmula, crecimiento e integridad de serie',
      () {
    ...
    test(
      'Proyección Mensual y Anual con verificación exacta de valores e integridad',
      () {
