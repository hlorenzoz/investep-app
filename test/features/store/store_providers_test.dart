...
void main() {
  ...
  test(
    'publicProductsProvider fetches active products and respects category filter',
    () async {
      when(
      ...
      when(
        () => mockStoreRepository.fetchProducts(category: 'book', active: true),
      ...
      container
          ...
          .setFilter(ProductCategory.book);
  ...
  test(
    'adminProductsProvider fetches all products without active filter',
    () async {
      when(
        () => mockStoreRepository.fetchProducts(),
      ).thenAnswer((_) async => testProducts);
  ...
  test(
    'storeAdminControllerProvider can create, update, and delete products',
    () async {
      ...
      when(
        () => mockStoreRepository.createProduct(newProductData),
      ).thenAnswer((_) async => testProducts[0]);
      when(
        () => mockStoreRepository.updateProduct(1, newProductData),
      ).thenAnswer((_) async => testProducts[0]);
      when(
        () => mockStoreRepository.deleteProduct(1),
